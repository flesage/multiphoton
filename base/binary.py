...
from excep import TimeoutError as TimeoutError, UnexpectedReplyError as UnexpectedReplyError
...
class BinaryDevice(object):
    ...
    def renumberAndCount(self):
        """Sends the "renumber" command (2), then waits for the device to
        reply.
        
        Returns: Number of devices.
        """
        
        #First, renumber devices and count how many there are (maximum 10)
        nDevices = 0
        command = BinaryCommand(0, 2, 0)
        self.port.write(command)
        prevTimeout = self.port.timeout
        self.port.timeout = 2
        for x in range(0, 10):
            try:
                reply1 = self.port.read()
                if len(reply1.encode()) == 6:
                    nDevices = nDevices + 1
                    #print "Device found!"  
            except Exception,e: 
                print str(e)
                self.port.timeout = prevTimeout
                return nDevices
        self.port.timeout = prevTimeout   
        return nDevices
...
class BinarySerial(object):
    ...
    def __init__(self, port, baud = 9600, timeout = 45):
        ...
        self._ser = serial.Serial(port, baud, timeout = timeout, interCharTimeout = 0.1) #Maxime: Change interCharTimeout to 0.1, was 0.01.
