...
class APTMotor(object):
    def __init__(self, SerialNum=None, HWTYPE=31, verbose=False):
        ...
        HWTYPE_KDC101       27  // added by PJM, to be tested
        ...
        #self.aptdll = windll.LoadLibrary('C:\\Users\\LiomW17\\Desktop\\APTMotor\\APT.dll')
        self.cleanUpAPT()
        ...
        # TODO : Error reporting to know if initialisation went sucessfully or not.
            ...
            if verbose: print "No serial for Thorlabs APT hardware, please setSerialNumber"
    ...
    def mAbs(self, absPosition):
        ...
        if self.configthorlabsmotor:
            
            if  (self.absPosition ) < self.maxtlabsmotor:
                #print(self.absPosition,'<',self.maxtlabsmotor)
                ...
                self.maxapt=True
                if self.verbose: print 'mAbs ', self.maxtlabsmotor, c_float(self.maxtlabsmotor)
                if not self.Connected:
                    raise Exception('Please connect first! Use initializeHardwareDevice')
                self.memmaxlabmotors=self.maxtlabsmotor
                self.maxtlabsmotor = c_float(self.maxtlabsmotor)
                self.aptdll.MOT_MoveAbsoluteEx(self.SerialNum, self.maxtlabsmotor, True)
                self.maxtlabsmotor=self.memmaxlabmotors
                if self.verbose: print 'mAbs SUCESS'
                return True
                print('Labjack is at maximum')
        else:    
            if self.verbose: print 'mAbs ', absPosition, c_float(absPosition)
            if not self.Connected:
                raise Exception('Please connect first! Use initializeHardwareDevice')
