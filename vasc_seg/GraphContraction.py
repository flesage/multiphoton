...
from util import getDist, assignPixelClusters, isSklNodes, cycleAreaAll, numpy_fill, createGraphFromSeg, cycleArea, fixG
...
class GraphContraction:
    ...
    def __computeMediality(self):
        ...
        dist_map=maximum_filter(dist_map, size=(3,3,3)) # apply max filter
    ...
    def __updateTopo(self, n=None, refine=False):
        ...
        if pos.shape[0]>0:
            ...
            centroids, _, poin_ind_= assignPixelClusters(pos) 
    ...
    def __connectSrg(self, G, pnts, centroids, skl_nodes):       
    ...
    def __applyContraction(self):
        ...
        ind111=np.array(range(len(n)))+2*len(n)
        ...
        a=a.tocoo()
        b=np.vstack([np.zeros_like(p), np.zeros_like(p), p*np.array([d,d,d]).T]) # b matrix
        
        print('Build A & B: '+str(time()-t0))
        
        t0=time()
        px_ = lsqr(a, b[:,0], atol=1e-05, btol=1e-05)[0] 
        py_ = lsqr(a, b[:,1], atol=1e-05, btol=1e-05)[0]
        pz_ = lsqr(a, b[:,2], atol=1e-05, btol=1e-05)[0]
    ...
    def generateGraph(self, 
        ...
        self.G_init=createGraphFromSeg(self.label, connect=8)
    ...
    def contractGraph(self,
        ...
        while chck or self.iteration<=self.n_iter:
            ...
            n_diff=self.__updateTopo(nodes_to_proc)
    ...
    def refineGraph(self, 
        ...
        while 1:
            ...
            self.__updateTopo(n=n, refine=True)   
