...
class GalvosController(QWidget, Ui_GalvosForm):
    ...
    def start_mozaic(self):
        ...
        while self.ending is False:
            ...
            self.thorlabs.mAbs(self.posfocusthorlabs)
            ...
            for i in range(int(math.ceil(self.nstepy))):
                ...
                for j in range(int(math.ceil(self.nstepx))):
                    ...
                    if self.imageprecheck[j,i,0] == 1 :
                        ...
                        nii_saver_a = liomio2.NiiStackSaver(config.filelocation,nx,ny,n_steps,n_extra,self.top,self.zstep,self.saving,self.imageprecheck,self.current_slice,i,j,1)
                        self.meas.setDataConsumer(nii_saver_a,True,1)
                    
                        time.sleep(0.1)
                    
                        nii_saver_b = liomio2.NiiStackSaver(config.filelocation,nx,ny,n_steps,n_extra,self.top,self.zstep,self.saving,self.imageprecheck,self.current_slice,i,j,0)
